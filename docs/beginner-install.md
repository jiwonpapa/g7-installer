...
- 리포트에 `completed`가 보입니다. 이 값은 패키지, Nginx/Apache/FrankenPHP 도메인 연결, PHP/DB 튜닝, DB 계정, SSL 처리, 앱 설치, 설정 안내서 저장까지 끝났다는 뜻입니다. 그누보드7/Laravel은 Composer, NPM, Artisan, queue, scheduler 서비스까지 구성하고, 그누보드7 Octane/Laravel Octane은 `g7-frankenphp` 워커 서비스까지 구성하며, WordPress는 설치 화면으로 이어집니다.
