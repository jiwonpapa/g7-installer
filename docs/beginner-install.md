...
- 서버 점검 단계가 보입니다.
- 기본 서버 구성 후 리포트에 `completed`가 보입니다. 이 값은 패키지, Nginx/Apache 도메인 연결, PHP-FPM/DB 튜닝, DB 계정, SSL 처리, 앱 설치 링크, 설정 안내서 저장까지 끝났다는 뜻입니다.
- 사이트 계정과 `/home/계정/public_html` 웹루트가 만들어집니다.
- `http://도메인` 접속이 됩니다.
- 서버에 `/var/log/g7-installer/setup-guide.md` 설정 안내서가 저장됩니다.
