...
목표는 실제 VPS에서 `g7inst` 웹 마법사를 열고 `서버 점검 -> 설치 방식 -> 사양 확정 -> 기본 서버 구성 -> 결과` 순서로 진행하는 것입니다.
...
- 서버 점검 단계가 보입니다.
- 기본 서버 구성 후 리포트에 `completed`가 보입니다. 이 값은 패키지, Nginx/Apache 도메인 연결, PHP-FPM/DB 튜닝, DB 계정, SSL 처리, 앱 설치, 설정 안내서 저장까지 끝났다는 뜻입니다. 그누보드7/Laravel은 Composer, NPM, Artisan, queue, scheduler 서비스까지 구성하고, WordPress는 설치 화면으로 이어집니다.
