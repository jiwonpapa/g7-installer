...
- 리포트에 `completed`가 보입니다. 이 값은 패키지, Nginx/Apache 도메인 연결, PHP/DB 튜닝, DB 계정, 앱 파일 배치, 설정 안내서 저장까지 끝났다는 뜻입니다. SSL은 DNS/IP 검증과 인증서 발급 조건이 맞을 때 적용되며, 보류나 실패가 있으면 리포트에 표시됩니다. 그누보드7은 Composer/NPM 준비, `.env`, APP_KEY, storage link, systemd unit 파일 준비까지 처리하고, 브라우저 `/install` 완료 뒤 queue, scheduler, Reverb 실행 여부를 세부 설정 액션 패널에서 확인합니다. WordPress는 설치 화면으로 이어집니다.
