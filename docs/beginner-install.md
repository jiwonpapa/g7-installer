...
다른 VPS에서 `sudo OK`가 나오지 않아도 `sudo` 권한이 있는 계정이면 일반 계정에서 setup을 실행할 수 있습니다. 설치기가 SSH 터미널에서 `sudo` 재실행을 시도합니다.

```bash
g7inst setup --domain example.com
```

sudo 비밀번호를 물어보면 SSH 터미널에 입력합니다. 웹 화면에는 root/ubuntu 서버 비밀번호를 입력하지 않습니다. sudo 자체가 실패하면 root SSH, `su -`, 또는 VPS 콘솔에서 관리자 권한을 먼저 확보해야 합니다.
...
- 리포트에 `completed`가 보입니다. 이 값은 패키지, Nginx/Apache 도메인 연결, PHP/DB 튜닝, DB 계정, 앱 파일 배치, 설정 안내서 저장까지 끝났다는 뜻입니다. SSL은 DNS/IP 검증과 인증서 발급 조건이 맞을 때 적용되며, 보류나 실패가 있으면 리포트에 표시됩니다. 그누보드7은 GitHub 공식 최신 안정 Release를 검증한 뒤 공식 `/install`로 넘깁니다. Composer/Vendor, `.env`, 관리자 계정, 확장과 마이그레이션은 G7 설치 화면에서 진행합니다. WordPress는 공식 최신 설치 화면으로 이어집니다.
...
| `sudo OK`가 안 나옴 | `g7inst setup --domain example.com`을 일반 계정에서 실행해 sudo 재실행을 시도하고, 안 되면 root SSH/`su -`/VPS 콘솔 확인 |
