...
`sudo g7 install --domain example.com` 실행 시:
...
- `/etc/nginx/sites-enabled/*`에 기존 사이트 설정 존재
- Apache 실행 중
...
```text
g7-installer-x86_64-unknown-linux-musl.tar.gz
g7-installer-aarch64-unknown-linux-musl.tar.gz
checksums.txt
bootstrap.sh
```
...
```bash
sudo g7 install --domain test.example.com
g7 status
...
- `sudo g7 install --domain example.com` 동작
- 기존 운영 서버 감지 시 안전 중단
- Nginx/PHP-FPM/MariaDB/G7 설치 완료
- HTTPS 적용 성공
- 설치 로그 저장
- `g7 doctor`, `g7 status`, `g7 logs` 동작
