...
> 현재 공개 릴리스는 `g7inst` 설치, 서버 점검, 웹 마법사, apt 패키지 설치, Nginx 도메인 연결 설정(vhost), PHP-FPM/DB 사양 튜닝, DB 앱 계정 생성, Nginx 기준 Let's Encrypt 인증서 발급/갱신 검증, 상세 설정 안내서 저장까지 검증합니다. 앱 소스 배치 자동화는 다음 배치입니다.
