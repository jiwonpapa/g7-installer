...
> 현재 `v0.2.3`는 설치기 바이너리 설치, 웹 컨트롤러 기반 설정, 서버 사전 점검, 설치 계획 생성, 설치 준비 파일 생성, 오류 힌트/상세 리포팅, 첫 token 접속 IP 잠금까지 검증합니다.
...
```bash
curl -fsSL https://raw.githubusercontent.com/jiwonpapa/g7-installer/main/scripts/bootstrap.sh | sudo env G7_INSTALL_VERSION=v0.2.3 bash
```
...
`Prepare install`은 한 번만 실행됩니다. 다시 테스트하려면 웹 컨트롤러를 종료한 뒤 `sudo g7inst reset --yes`로 준비 흔적을 지우고 다시 실행합니다.
...
웹 UI에서 서버 계정 로그인, 서버 점검, 옵션 선택, 계획 확인, 설치 준비, 리포트 확인 순서로 진행합니다. 설치 준비 중에는 중복 실행을 차단합니다.
...
설치 중 오류가 나면 웹 UI의 Live log와 Report 영역에 서버 오류, 해결 힌트, 실패한 doctor 항목 상세가 같이 표시됩니다. 먼저 `Server check`를 다시 실행해 실패 항목을 확인하고, 이전 테스트 흔적이면 `reset --yes`로 지운 뒤 재시도합니다.
...
```text
https://github.com/jiwonpapa/g7-installer/releases/tag/v0.2.3
```
