...
> 현재 `v0.2.7`는 설치기 바이너리 설치, 웹 마법사 기반 설정, 라이트/다크 테마, 항목별 도움말, 서버 사전 점검, 설치 계획 생성, 실제 apt 패키지 설치, 기본 서비스/포트 검증, 설치 직후 패키지 되돌리기, 오류 힌트/상세 리포팅, 첫 token 접속 IP 잠금까지 검증합니다.
