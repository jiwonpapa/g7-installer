...
> 현재 공개 릴리스는 `g7inst` 설치, 서버 점검, 웹 마법사, apt 패키지 설치, 진행률, 리포트, 패키지 되돌리기까지 검증합니다. vhost 적용, DB 앱 계정 생성, 앱 소스 배치, Certbot 발급은 다음 단계입니다.
...
`rollback`은 설치 직후 되돌리기용입니다. 운영 중인 사이트 백업 복구 기능이 아닙니다.
