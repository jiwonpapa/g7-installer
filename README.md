...
`completed`는 **서버 프로비저닝 완료**를 뜻합니다. 그누보드7과 WordPress의 CMS 내부 설정, 테이블 구성, 관리자 계정 생성은 결과 리포트의 앱 링크에서 각 공식 브라우저 설치기를 한 번 더 진행해야 완료됩니다.
...
서버, 도메인, 방화벽 설정과 `g7inst` bootstrap 설치가 끝났다면 아래 순서대로 실행합니다. 아직 `g7inst`가 없다면 [따라하기식 설치 매뉴얼](docs/copy-paste-install.md)의 4번부터 진행합니다.
...
결과 리포트의 `앱 링크`가 G7이면 `/install`, WordPress이면 `/wp-admin/install.php`입니다. 이 링크를 열어 공식 설치 화면을 마쳐야 CMS 설치가 끝납니다. URL은 `www` 사용 여부와 HTTPS 적용 결과를 반영하므로 직접 조합하지 말고 리포트의 링크를 사용합니다.
