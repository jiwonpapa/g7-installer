...
> 현재 `v0.2.4`는 설치기 바이너리 설치, 웹 마법사 기반 설정, 라이트/다크 테마, 항목별 도움말, 서버 사전 점검, 설치 계획 생성, 설치 준비 파일 생성, 오류 힌트/상세 리포팅, 첫 token 접속 IP 잠금까지 검증합니다.
...
```bash
curl -fsSL https://raw.githubusercontent.com/jiwonpapa/g7-installer/main/scripts/bootstrap.sh | sudo env G7_INSTALL_VERSION=v0.2.4 bash
```
...
웹 UI의 `설치 준비 시작`은 한 번만 실행됩니다. 다시 테스트하려면 웹 컨트롤러를 종료한 뒤 `sudo g7inst reset --yes`로 준비 흔적을 지우고 다시 실행합니다.
...
웹 UI에서 서버 계정 로그인, 서버 점검, 설치 방식 선택, 사양 확정, 설치 준비, 결과 확인 순서로 진행합니다. 사양 확정 단계에서 계획 생성이 성공해야 `이 사양으로 진행` 버튼이 열리고, 그 다음 `설치 준비 시작`을 실행합니다. 각 단계는 `이전` 버튼과 브라우저 뒤로가기로 돌아갈 수 있습니다.

기본 화면은 초보자용 마법사이며, 어려운 용어 옆의 `?` 버튼으로 도움말을 확인할 수 있습니다. 상세 설정은 접힘 영역에 숨겨져 있고, 라이트/다크 테마를 선택할 수 있습니다.
...
설치 중 오류가 나면 웹 UI의 상세 로그와 결과 리포트 영역에 서버 오류, 해결 힌트, 실패한 점검 항목 상세가 같이 표시됩니다. 먼저 `서버 점검`을 다시 실행해 실패 항목을 확인하고, 이전 테스트 흔적이면 `reset --yes`로 지운 뒤 재시도합니다.
...
현재 빌드는 원격 공개 바인딩에서 서버 계정 비밀번호 로그인을 차단합니다. 서버 계정 로그인과 설치 준비 액션은 기본 `127.0.0.1` 바인딩에서 SSH 터널로 접속해 사용하세요.
...
- `g7inst setup`
  ...
  - WebSocket으로 Live log와 단계별 진행 상태 표시
  - 서버 점검, 옵션 선택, 계획 확인, 설치 준비, 리포트, 리셋 화면 제공
...
- `sudo g7inst install --domain example.com`
  - 현재는 MVP 준비 단계입니다.
  - 실제 apt/Nginx/PHP/MySQL/Certbot/Redis/SSH 설정 전 단계까지만 수행합니다.
...
```text
https://github.com/jiwonpapa/g7-installer/releases/tag/v0.2.4
```
