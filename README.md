...
Lightsail `Add launch script`에는 아래 스크립트를 넣습니다. 이 스크립트는 `g7inst` 설치까지만 처리합니다. OS 업데이트, swap, UFW, fail2ban, SSH 보안 점검, 웹서버, PHP, DB, Redis, Certbot, 앱 설치는 `g7inst setup` 웹 UI가 처리합니다.
...
cat >/opt/g7-bootstrap/README.txt <<'BOOTSTRAP_README'
...
`.pub` 공개키는 Lightsail 콘솔에 등록하거나 위 스크립트의 `EXTRA_SSH_PUBLIC_KEY`에 넣을 수 있습니다. 개인키는 서버, Git, 문서, 시작 스크립트에 넣지 않습니다.
