...
> 현재 `v0.2.7`는 설치기 바이너리 설치, 웹 마법사 기반 설정, 라이트/다크 테마, 잘리지 않는 항목별 도움말, 서버 사전 점검 통과 후 다음 단계 진입, 설치 계획 생성, 실제 apt 패키지 설치, 진행률 표시, HTML 결과 리포트, 설치 직후 패키지 되돌리기, 오류 힌트/상세 리포팅, 첫 token 접속 IP 잠금까지 검증합니다.
...
웹 UI의 앱 선택은 `그누보드7`, `WordPress`, `Laravel`을 제공합니다. 현재 단계에서는 서버 스택 준비와 계획/리포트에 반영되며, 실제 앱 소스 다운로드, DB 설정 파일 생성, vhost 연결은 후속 설치 단계에서 구현합니다.
...
app package: gnuboard7, wordpress, laravel (web UI 표시/후속 앱 설치용)
