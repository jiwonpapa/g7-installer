...
> 현재 `v0.2.5`는 설치기 바이너리 설치, 웹 마법사 기반 설정, 라이트/다크 테마, 항목별 도움말, 서버 사전 점검, 설치 계획 생성, 실제 apt 패키지 설치, 기본 서비스/포트 검증, 오류 힌트/상세 리포팅, 첫 token 접속 IP 잠금까지 검증합니다.
...
```bash
curl -fsSL https://raw.githubusercontent.com/jiwonpapa/g7-installer/main/scripts/bootstrap.sh | sudo env G7_INSTALL_VERSION=v0.2.5 bash
```
...
웹 UI의 `패키지 설치 시작`은 한 번만 실행됩니다. 다시 테스트하려면 웹 컨트롤러를 종료한 뒤 `sudo g7inst reset --yes`로 installer 메타데이터를 지우고 다시 실행합니다. 이미 설치된 apt 패키지는 자동 제거하지 않습니다.
...
웹 UI에서 서버 계정 로그인, 서버 점검, 설치 방식 선택, 사양 확정, 패키지 설치, 결과 확인 순서로 진행합니다. 사양 확정 단계에서 계획 생성이 성공해야 `이 사양으로 진행` 버튼이 열리고, 그 다음 `패키지 설치 시작`을 실행합니다. 각 단계는 `이전` 버튼과 브라우저 뒤로가기로 돌아갈 수 있습니다.
...
현재 `install`은 apt 패키지 설치, 기본 서비스 시작, 패키지/서비스/포트 검증까지 수행합니다. 생성되는 핵심 파일은 `/etc/g7-installer/config.toml`, `/var/lib/g7-installer/state.json`, `/var/lib/g7-installer/owned-files.json`, `/var/log/g7-installer/report.json`입니다. 앱 웹루트는 config/report에 기록만 하고 아직 생성하지 않습니다.
...
- `g7inst setup`
  ...
  - 서버 점검, 옵션 선택, 계획 확인, 패키지 설치, 리포트, 리셋 화면 제공
...
```text
https://github.com/jiwonpapa/g7-installer/releases/tag/v0.2.5
```
